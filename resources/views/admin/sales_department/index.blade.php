                    ...
                    <tr>
                        ...
                        <th class="no-sort" scope="col">
                            <label class="checkboxs">
                                <input type="checkbox" id="select-all" class="sales_department_checkbox">
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="salesDepartmentForm">
                    @csrf
                    <input type="hidden" name="sales_department_id">
                    <div class="mb-3">
...
@section('script')
<script>
    var department_table_show = $('#department_table').DataTable({
        "pageLength": 10,
        ...
        dom: 'lrtip',
        ajax: {
            url: '{{ route('sales_department.index') }}',
            data: function(d) {
    ...
    $('#customSearch').on('keyup', function() {
        department_table_show.search(this.value).draw();
    });
    ...
    $('#openModal').click(function() {
        $('#salesDepartmentForm')[0].reset();
        $('#adminModal').modal('show');
        $('#modalTitle').text('Add Department');
        $('#submitBtn').text('Create');
        $('input[name="category_id"]').val('');
        // $('select[name="parent_category_id"]').parent().show();
        $("#salesDepartmentForm .text-danger").text('');
        $('#salesDepartmentForm').find('.is-invalid').removeClass('is-invalid');
    });


    /***** Open Modal for Editing an Admin *****/
    $(document).on('click', '.edit-btn', function() {
        let sales_department_id = $(this).data('id');
        // alert(category_id);
        $("#salesDepartmentForm .text-danger").text('');
        $('#salesDepartmentForm').find('.is-invalid').removeClass('is-invalid');

        $.get('{{ route('sales_department.edit', ':id') }}'.replace(':id', sales_department_id), function(sales_department) {
            console.log(sales_department);
            $('#modalTitle').text('Edit Department');
            $('#submitBtn').text('Update');
            $('input[name="sales_department_id"]').val(sales_department_id);

            $('input[name="name"]').val(sales_department.name);
            $('input[name="status"][value="' + sales_department.status + '"]').prop('checked', true);
            $('#adminModal').modal('show');
        });
    });

    /***** Add & Edit Form Submission *****/
    $('#salesDepartmentForm').submit(function(e) {
        e.preventDefault();
        let sales_department_id = $('input[name="sales_department_id"]').val();
        let url = sales_department_id ? '{{ route('sales_department.update', ':id') }}'.replace(':id', sales_department_id) :
            "{{ route('sales_department.store') }}";
        let method = sales_department_id ? "PUT" : "POST";
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                department_table_show.ajax.reload();
    ...
    function confirmDeletion(callback) {
        Swal.fire({
            title: "Are you sure?",
            text: "You want to remove this Department? Once deleted, it cannot be recovered.",
            icon: 'warning',
    ...
    /***** Delete *****/
    $(document).on('click', '.deleteSalesDepartment', function(event) {
        event.preventDefault();
        let categoryId = $(this).data('id');
        let form = $('#delete-sales-department-form-' + categoryId); // Select the correct form
    ...
    function display_errors(errors) {
        $("#salesDepartmentForm .error-text").text('');
    ...
    $('#select-all').change(function() {
        ...
        $('.sales_department_checkbox').prop('checked', this.checked);

    });

    $(document).on('change', '.sales_department_checkbox', function() {
        let count = $('.sales_department_checkbox:checked').length; // Count checked checkboxes
    ...
    $('#bulk_delete_button').click(function() {
        confirmDeletion(function() {
            var selectedIds = $('.sales_department_checkbox:checked').map(function() {
                return $(this).data('id');
            }).get();
            ...
            if (selectedIds.length > 0) {
                ...
                $.ajax({
                    url: "{{ route('sales_department.bulkDelete') }}",
                    method: 'POST',
                    data: {
